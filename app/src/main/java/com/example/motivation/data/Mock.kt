...
class Mock {
    ...
    private val langFr = MotivationConstants.LANGUAGE.FRANCE
    ...
    fun getPhrase(categoryId: Int,lang:String) : String{
        return if(categoryId == MotivationConstants.FILTER.ALL){
            val filteredPhrase = mListPhrase.filter{it.lang == lang}
            val randomId = Random.nextInt(filteredPhrase.size)
            filteredPhrase[randomId].description
        }else{
            val filteredPhrase = mListPhrase.filter {it.categoryId == categoryId && it.lang == lang }
            val randomId = Random.nextInt(filteredPhrase.size)
            filteredPhrase[randomId].description
        }
    }
